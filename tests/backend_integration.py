...
try:
    ...
    check(b.data['state']['conversations'][aid]['unreadCount']==1,'new message unread count');mid=b.data['state']['conversations'][aid]['messages'][-1]['id']
    check(any(n['kind']=='message' and 'mensajes.html' in n['href'] for n in b.data['data']['notifications']),'message notification links to chat')
    b.post('message-read',{'id':aid,'lastId':mid});check(b.status==200 and b.data['snapshot']['state']['conversations'][aid]['unreadCount']==0,'opening chat marks incoming message read')
    ...
    b.post('profile',{'fields':{'name':'Integration 1','handle':f'test_{suffix}_1','profileAccent':'ocean','profileBorder':'glow'}});check(b.status==200 and b.data['snapshot']['data']['creators']['demo']['profileAccent']=='ocean','Plus profile customization saved')
    a.post('profile',{'fields':{'name':'Integration 0','handle':f'test_{suffix}_0','profileAccent':'ocean'}});check(a.status==403,'non Plus cannot customize profile')
    ...
    b.get('state');check(not any(s['id']==sid for s in b.data['community']['stories']),'unfollowed stories hidden')
    ...
    a.get('state');reply=a.data['state']['conversations'][bid]['messages'][-1];check(reply['storyReply'] and reply['story']['id']==sid,'story reply retains a playable preview')
    ...
    a.get('file&id='+story['id']);check(a.status==200,'owner can access archived media')
    cover=a.upload();a.post('highlight-save',{'name':'Memories','stories':[sid],'coverAsset':cover['id']});check(a.status==200,'create highlight with cover')
    hid=next(h['id'] for h in a.data['snapshot']['community']['highlights'] if sid in h['stories'])
    b.post('highlight-save',{'id':hid,'name':'Stolen','stories':[sid]});check(b.status==403,'highlight ownership enforced')
    b.get('file&id='+story['id']);check(b.status==200,'highlight keeps expired story accessible')
    anon.get('file&id='+cover['id']);check(anon.status==200,'highlight cover public')
    a.post('highlight-save',{'id':hid,'name':'Renamed','stories':[sid],'coverAsset':None});check(a.status==200,'highlight rename and remove cover')
    a.post('highlight-delete',{'id':hid});check(a.status==200,'owner removes highlight group')
    b.get('file&id='+story['id']);check(b.status==403,'removing final group restores story expiry')
    secretstory=a.upload();a.post('publish-story',{'text':'Private saved memory','media':[secretstory],'visibility':'subscribers'});check(a.status==200,'private story created')
    private_sid=next(s['id'] for s in a.data['snapshot']['community']['stories'] if s['text']=='Private saved memory')
    a.post('highlight-save',{'name':'Private','stories':[private_sid]});check(a.status==200,'private highlight created')
    anon.get('file&id='+secretstory['id']);check(anon.status==403,'private highlight file denied to guest')
    anon.get('state');check('Private saved memory' not in json.dumps(anon.data),'private highlight text redacted')
    b.get('file&id='+secretstory['id']);check(b.status==200,'subscriber can read private highlight')
    a.post('delete-story',{'id':private_sid});check(a.status==200,'private highlighted story removed')
    ...
    anon.post('google',{'idToken':'fake.token.signature'});check(anon.status==401,'fake Google token rejected')
