(() => {
    ...
    function post(p, expanded = false) {
        ...
        let attachment = '';
        if (!readable) attachment = `<button class="locked-media" data-action="subscribe" data-creator="${p.creatorId}"><span class="lock-badge">${icon('lock')}</span><strong>Contenido Exclusivo</strong><span>Suscríbete a ${escape(creator.name)} para ver esta publicación.</span><span class="button-primary">Desbloquear · $4.99 / mes</span></button>`;
    ...
    window.FansxeComponents = { escape, icon, number, post, avatar, media, video, richText, profile, profileUrl, tabs, person };
})();
