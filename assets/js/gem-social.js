(() => {
    ...
    $('gem-tip-form').querySelector('.field-help').insertAdjacentHTML('beforebegin',`<label class="form-label" for="gem-tip-text">Un mensaje para el creador <small>Opcional</small></label><textarea id="gem-tip-text" class="text-field" maxlength="500" rows="3" placeholder="Gracias por tu contenido… o cuéntale qué tienes en mente"></textarea><label class="attachment-button tip-photo-label" for="gem-tip-photo">${ui.icon('photo')} Añadir una foto</label><input id="gem-tip-photo" type="file" class="sr-only" accept="image/png,image/jpeg,image/webp,image/gif"><div id="gem-tip-preview" class="attachment-preview"></div><p class="field-help">Tu propina, mensaje y foto llegarán a su chat privado. Foto opcional, hasta 8 MB.</p>`);
    const tipPicker=FansxeAttachments.create($('gem-tip-photo'),$('gem-tip-preview'),app.notify,true,1,true);
    function openTip(id,context='profile',post=null){
        ...
        tipPicker.clear();$('gem-tip-text').value='';$('gem-tip-form').querySelectorAll('input,textarea,button').forEach(el=>el.disabled=false);$('gem-tip-confirm').textContent='Confirmar propina';
    ...
    $('gem-tip-form').addEventListener('submit',async ev=>{
        ...
        sending=true;$('gem-tip-form').querySelectorAll('input,textarea,button').forEach(el=>el.disabled=true);const key=gift.requestKey;
        try {
            if(!gift.payload){const media=await tipPicker.save();gift.payload={id:gift.id,context:gift.context,post:gift.post,requestKey:key,gems,text:$('gem-tip-text').value,media};}
            const ok=await store.write('gem-tip',gift.payload);
            if(ok){if(gift?.requestKey===key){gift=null;tipPicker.clear();if(app.activeModal==='gemTipModal')app.closeModal();}app.notify(`Propina de ${fmt(gems)} gemas enviada al chat.`);}
            else $('gem-tip-confirm').textContent='Reintentar el mismo envío';
        }catch(error){app.notify(error.message);if(!gift?.payload)$('gem-tip-form').querySelectorAll('input,textarea,button').forEach(el=>el.disabled=false);}
