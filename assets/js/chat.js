(() => {
    ...
    function renderMessages(scroll = false) {
        ...
        target.innerHTML = conversation?.messages.length ? conversation.messages.map(m => `<article class="message-bubble ${m.senderId === 'demo' ? 'message-own' : 'message-incoming'}">${m.storyReply ? storyPreview(m.story) : ''}${m.gemTip ? `<div class="message-gem-tip">${ui.icon('gem')}<span>${ui.number(m.gemTip.gems)} gemas · Propina</span></div>${m.text ? `<p>${ui.richText(m.text)}</p>` : ''}` : `<p>${ui.richText(m.text)}</p>`}${ui.media(m.media)}<small>${ui.escape(new Date(m.createdAt).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' }))}${window.FansxeBoot ? '' : ' · Guardado localmente'}</small></article>`).join('') : '<div class="chat-start"><span class="empty-icon">♡</span><h3>Comienza la conversación</h3><p>Envía un saludo, una foto o un video.</p></div>';
        markRead();
